...
class menuManager extends msDB {
	...
	function save(
				...
				$conf = Array(); 
				if ($menu_id) {
					$strSql = "update menu set title=?,iconCls=?,handler=?,ajax=?,published=? where id=?"; 
					$args = Array(
									$menu_title,
									$iconCls,
									$handler,
					...
					$strSql = "insert into menu (parent_id,title,iconcls,handler,ajax,published)"; 
					$strSql .= " values(?,?,?,?,?,?);";
					$args = Array(
									$parent_id,
									$menu_title,
	...
	function handlerList() {
		$result = $this->fileList("layouts/","js"); 
		return $result; 
	}
