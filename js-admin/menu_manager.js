...
var detailMenu = {
	...
	items : [{
				...
				xtype : 'trigger',
				fieldLabel : 'Java Script Handler',
				anchor : '95%',
				name : 'handler',
				allowBlank : true,
				triggerConfig : {
...
var proxyIcon = new Ext.data.HttpProxy({
			api : {
				read : ajax_url + '&task=gridIcon&action=doRead',
				create : ajax_url + '&task=gridIcon&action=doCreate',
				update : ajax_url + '&task=gridIcon&action=doUpdate',
				destroy : ajax_url + '&task=gridIcon&action=doDestroy'
			}
		});
...
function showHandler(btn) {
	handler_select = (btn.fieldLabel == 'Ajax Handler') ? 'php' : 'js';
