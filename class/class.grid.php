...
class grid extends msDB {
	...
	function sendParam($REQUEST) {
		$this->start = ($REQUEST["start"] == null)? 0 : $REQUEST["start"];
		$this->count = ($REQUEST["limit"] == null)? 30 : $REQUEST["limit"];
		$this->sort = ($REQUEST["sort"] == null)? "" : $REQUEST["sort"];
	...
	function isMasterField($field) {
		$result = true; 
		if ($this->join) {
			$result = preg_match("/\b".$this->master_table."\b/i",$this->getKeyField($field));
		}
		return $result; 
	}
