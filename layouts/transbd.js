...
var filterstatus = [];
dsstatus.each(function(r,i){
	filterstatus.push(r.data.status);
});					
...
var ds = new Ext.data.GroupingStore({
			...
			remoteSort : true,
                        ...
                        listeners:{
                            load:function(){
                                Ext.getCmp('txt_total').setValue(this.getAt(0).data.total);
                            }
                        }
...
var expander = new Ext.ux.grid.RowExpander({
    tpl : new Ext.XTemplate(
		 ...
		 "<span>No. Bukti Potong : {potput} </span><br />",
		 "<span>Pengembalian :  {[fm.number(values.retur,'Rp  0.000,00/i')]} </span><br />",
         "</div>"
    )
});
...
Ext.ux.grid.GroupSummary.Calculations['totalReal'] = function(v, record, field){
        return v + (record.data.jml - record.data.retur);
    };
	...
	columns : [expander,
            {
                ...
                dataIndex:'retur',
                header:'Pengembalian',
				renderer: function(val,cell) {cell.attr = 'style=color:red'; return '' + Ext.util.Format.number(val,'0.000,00/i');},
                align: 'right',
				width: 100,
                summaryType: 'sum',
                summaryRenderer:function(val){
                    x = Ext.util.Format.number(val,'0.000,00/i');
                    return String.format("<span style=color:red><b>{0}</b></span>",x);
                }
				...
				renderer: function(val,cell) {cell.attr = 'style=color:red'; return '' + Ext.util.Format.number(val,'0.000,00/i');},
                align: 'right',
				renderer:  function(val,cell,record){cell.attr = 'style=color:blue'; x = record.data.jml - record.data.retur; 
...
var grid = new Ext.grid.GridPanel({
	title : 'Daftar Rekap Transaksi Bendahara',
	// region:'center',
	iconCls : 'parent-form',
	...
	loadMask : true,
	plugins : [filter,summary,expander],
        tbar: [{
                text:'Tambah Data',
                iconCls:'add-data',
                tooltip:{
					...
					tooltip : {
								...
								items : [{
											...
											.setIconClass('report-word');
                                                                                        ...
                                                                                        if (!grid.getSelectionModel().getSelected())
                                                                                            return false;
                                                                                        id_data = grid.getSelectionModel().getSelected().data.id;
                          ...
                          'Total ',{
                              ...
                              listeners:{
                              render:function(){
                                  //this.setValue(ds.getAt(0).data.total);
                              }}
                          } ],
...
var retur = new Ext.ux.customRendererField({
            fieldLabel: 'Pengembalian',
            name: 'retur',
		//	customRenderer:cr,
            width:175
...
var item_form = [{
		...
		, {
			...
			fieldLabel : 'Alamat',
			name : 'alamat',
			value:'Jl. DR. Wahidin Raya No.1 Gedug Djuanda I Lt 18',
			width:200
		}, {
			xtype : 'hidden',
	...
	buttons : [{
		...
		handler : function() {
			if (gridWindowspeny.getSelectionModel().hasSelection()) {
				...
				frm_edit.getForm().setValues({subject: data.nama, npwp: data.npwp, alamat: data.alamat, norek:data.norek, bank:data.bank});
				winGridpeny.hide();
				frm_edit.body.highlight('#c3daf9', {
							block : true
				});
